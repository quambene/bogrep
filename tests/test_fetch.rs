...
async fn test_fetch_diff() {
    let mock_server = common::start_mock_server().await;
    let (mocks, mock_guard) = common::mount_mock_scoped(&mock_server, 1, 1).await;
    let temp_dir = tempdir().unwrap();
    let temp_path = temp_dir.path();
    assert!(temp_path.exists(), "Missing path: {}", temp_path.display());
    let source_path = temp_path.join("test_data");
    ...
    // Change the content for the endpoint to simulate a diff.
    drop(mock_guard);
    let (mocks, _mock_guard) = common::mount_mock_scoped(&mock_server, 1, 2).await;
    for url in mocks.keys() {
        writeln!(file, "{}", url).unwrap();
    }
    ...
    let mut cmd = Command::cargo_bin(env!("CARGO_PKG_NAME")).unwrap();
    cmd.env("BOGREP_HOME", temp_path);
    cmd.args(["fetch", "--diff", &bookmarks[0].url]);
    cmd.assert()
        .success()
        .stdout(str::contains("-Test content 1+Test content 2"));
}
